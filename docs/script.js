...
class TerminalDemo {
  constructor(element) {
    ...
    this.lines = [
      { prompt: '$ ', command: 'brain save "debugging auth middleware - tokens expiring randomly"', output: ['📝 Context saved successfully!', '🧠 AI analysis complete', '✨ Ready to resume anytime'] },
      { prompt: '$ ', command: 'cd ~/work && git checkout feature/payment-flow', output: ['Switched to branch "feature/payment-flow"'] },
      { prompt: '$ ', command: 'brain resume', output: [
  ...
  async typeLine(line) {
    ...
    // Type prompt
    lineElement.innerHTML = `<span class="terminal-prompt">${line.prompt}</span>`;
    
    // Type command
    for (let char of line.command) {
      await this.wait(50 + Math.random() * 50);
      lineElement.innerHTML = `<span class="terminal-prompt">${line.prompt}</span><span class="terminal-command">${line.command.substring(0, lineElement.textContent.length - line.prompt.length + 1)}</span>`;
    }
    ...
    for (let output of line.output) {
      ...
      outputElement.innerHTML = `<span class="terminal-output">${output}</span>`;
...
class GitHubStats {
  ...
  async fetchStats() {
    try {
      const response = await fetch(`https://api.github.com/repos/${this.repoUrl}`);
