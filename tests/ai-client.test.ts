...
import type { GitContext, AIInterpretation } from "../src/storage/models.ts";
...
Deno.test("AIClient should generate correct prompt format", () => {
  ...
  assertStringIncludes(prompt, "debugging auth middleware - tokens expiring randomly");
...
Deno.test("AIClient should handle successful OpenAI response", async () => {
  ...
  const result = await client.analyzeContext("debugging auth middleware", mockGitContext);
  
  assertEquals(result.summary, "Working on authentication flow issues with token expiry");
  assertEquals(result.suggestedNextSteps.length, 3);
  assertEquals(result.confidenceScore, 0.85);
  assertEquals(result.relatedFiles, ["auth/middleware.js", "config/jwt.js", "tests/auth.test.js"]);
